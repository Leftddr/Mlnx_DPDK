...
mlx5_rx_burst(void *dpdk_rxq, struct rte_mbuf **pkts, uint16_t pkts_n)
{
	...
	while (pkts_n) {
		...
		if (pkt)
			NEXT(seg) = rep;
		seg = rep;
		//rx queue에 dma로 바로 써주니까 여기서는 바로 읽어오는 역할을 하는 건가?
		rte_prefetch0(seg);
		rte_prefetch0(cqe);
		rte_prefetch0(wqe);
		/* Allocate the buf from the same pool. */
		//seq에 rep의 값을 넣어놓고, rep에는 새로운 buf를 할당한다
		//새로운 mbuf를 할당하기 위한 함수이다.
		rep = rte_mbuf_raw_alloc(seg->pool);
		//unlikely rep == NULL이 되지 않을 가능성이 높다는 걸 알려주면서 성능을 높이다.
		//compiler가 이에 따라 compile시에 코드를 최적화 한다.
		if (unlikely(rep == NULL)) {
			...
			/*
				...
				NEXT(pkt) = NULL;
				pkt = rep;
				circular queue라는 것을 잊으면 안된다.
				위의 일을 반복하면서 pkt를 free시킨다.
			*/
			while (pkt != seg) {
			...
			/*
				sges_n -> 한 패킷당 필요한 segment의 개수.
			*/
			rq_ci >>= sges_n;
			++rq_ci;
			rq_ci <<= sges_n;
			//더 이상 공간이 존재하지 않을 때 while 문을 빠져 나간다/
			break;
		}
		if (!pkt) {
			/*
				idx는 무슨 queue이냐에 따라, 
				cq_ci & cqe_cnt => mellanox에서 사용하는 completeion queue index
				rq_ci & wqe_cnt => 일반적인 receive queue index를 의미한다. 
				로 사용할 수 있다.
			*/
			cqe = &(*rxq->cqes)[rxq->cq_ci & cqe_cnt];
			len = mlx5_rx_poll_len(rxq, cqe, cqe_cnt, &mcqe);
			//len이 0이면 rep를 free한다. => 다음에 진행시킬 packet이 없다는 의미이다.
			if (!len) {
				rte_mbuf_raw_free(rep);
				break;
			}
			// 이 줄 때문에 segment가 packet를 의미한다는 것을 알 수 있다.
			pkt = seg;
		  ...
		  unsigned int olx)
{
	...
	static_assert(MLX5_ESEG_MIN_INLINE_SIZE ==
		      ...
		      "invalid Ethernet Segment data size");
	/////////////////////////////////////////////////////////////
	//loc->mbuf에서 psrc로 data를 추출한다.
	psrc = rte_pktmbuf_mtod(loc->mbuf, uint8_t *);
	//header size를 계산한다.
	es->inline_hdr_sz = rte_cpu_to_be_16(inlen);
	//es->inline_data에 psrc에 넣어놓은 data를 넣어놓는다.
	es->inline_data = *(unaligned_uint16_t *)psrc;
	psrc +=	sizeof(uint16_t);
	pdst = (uint8_t *)(es + 1);
	if (MLX5_TXOFF_CONFIG(VLAN) && vlan) {
			  ...
			  unsigned int olx)
{
	...
	for (;;) {
		...
		// inline flag가 setting 되어 있으면, ethernet segment안에 data를 inline 시켜서 보낸다.
		if (MLX5_TXOFF_CONFIG(INLINE)) {
			unsigned int inlen, vlan = 0;
			//inlen은 mbuf->len, data의 길이를 의미한다.
			inlen = rte_pktmbuf_data_len(loc->mbuf);
			if (MLX5_TXOFF_CONFIG(VLAN) &&
			    //보내야 될 packet의 size를 check한다.
			    //struct rte_vlan_hdr + struct rte_ether_hdr
			    loc->mbuf->ol_flags & PKT_TX_VLAN_PKT) {
				vlan = sizeof(struct rte_vlan_hdr);
				inlen += vlan;
				static_assert((sizeof(struct rte_vlan_hdr) +
					       sizeof(struct rte_ether_hdr)) ==
					       // minimal ehternet inline segment size를 의미한다.
					       MLX5_ESEG_MIN_INLINE_SIZE,
...
single_inline:
				...
				wqe = txq->wqes + (txq->wqe_ci & txq->wqe_m);
				loc->wqe_last = wqe;
				// Control Segment를 만든다.
				mlx5_tx_cseg_init(txq, loc, wqe, seg_n,
						  MLX5_OPCODE_SEND, olx);
				// inlining data를 넣은 Ethernet Segment를 만든다.
...
single_no_inline:
			...
			mlx5_tx_dseg_ptr
				(txq, loc, &wqe->dseg[0],
				//rte_pktmbuf_mtod : 데이터의 시작 부분의 pointer를 돌려준다.
				//생각해 볼 때, 한 1byte되는 것 같다..
				 rte_pktmbuf_mtod(loc->mbuf, uint8_t *),
				 rte_pktmbuf_data_len(loc->mbuf), olx);
			++txq->wqe_ci;
			--loc->wqe_free;
