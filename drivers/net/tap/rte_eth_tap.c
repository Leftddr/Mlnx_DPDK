...
pmd_rx_burst(void *queue, struct rte_mbuf **bufs, uint16_t nb_pkts)
{
	...
	//last seen trigger와 trigger의 값이 다르면 시행한다.
	...
	for (num_rx = 0; num_rx < nb_pkts; ) {
		...
		while (1) {
			...
			//segment와 mbuf의 차이를 알아야 되는데 둘이 다른게 없는 건가?
			//여기선 seg도 rte_mbuf이고 mbuf도 rte_mbuf이다.
